...
void main() {
  group('OTel surface coverage guards', () {
    test('every presentation screen is wrapped in ObservableScreen', () {
      final screenFiles =
          Directory('lib/ui/product_surfaces')
      ...
      expect(
        ...
        isEmpty,
        reason:
            'Every screen-level surface must be externally observable. '
    ...
    test('interactive screen files declare ObservableInteraction coverage', () {
      ...
      final files = <File>[
        ...
        File('lib/ui/product_surfaces/app/tab_shell.dart'),
        File('lib/ui/product_surfaces/map/widgets/hierarchy_header.dart'),
      ]..sort((a, b) => a.path.compareTo(b.path));

      final missing = <String>[];
      for (final file in files) {
        final source = file.readAsStringSync();
        final hasInteractiveCallback = interactivePatterns.any(
          (pattern) => pattern.hasMatch(source),
        );
        if (!hasInteractiveCallback) continue;

        final hasCoverage =
            source.contains('ObservableInteraction.') ||
      ...
      expect(
        ...
        isEmpty,
        reason:
            'Files with user-interaction callbacks must route those '
    ...
    test(
      'domain use cases are observable and propagate trace ids downstream',
      () {
        final useCaseFiles =
            Directory('lib/features')
                .listSync(recursive: true)
        ...
        for (final file in useCaseFiles) {
          ...
          for (final match in repositoryCalls) {
            ...
            final callSource = callEnd == -1
                ? tail
                : tail.substring(0, callEnd);
        ...
        expect(
          ...
          isEmpty,
          reason:
              'Every domain use case must extend ObservableUseCase.\n'
              '${notObservable.join('\n')}',
        );
        expect(
          droppedTrace,
          isEmpty,
          reason:
              'Use cases that call repositories must propagate traceId so '
    ...
    test(
      'Supabase repositories emit operation, duration, and error telemetry',
      () {
        final repositoryFiles =
            Directory('lib/features')
                ...
                .where(
                  (file) => file.path.split('/').last.startsWith('supabase_'),
                )
        ...
        expect(
          ...
          isEmpty,
          reason:
              'Supabase repository calls are external system boundaries. '
