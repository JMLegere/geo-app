...
final class FakeLivingWorldRepository implements LivingWorldRepository {
  ...
  @override
  Future<TownProjection> readTown(String playerId,
      {required String traceId}) async {
    reads += 1;
...
void main() {
  group('TownNotifier', () {
    test('loads, refreshes, invalidates, and recreates durable Town state',
        () async {
      ...
      expect(
          container.read(townProvider).town!.venues.single.villagers, isEmpty);
    ...
    test(
      ...
      repository.onRecord = (_) async =>
          LivingWorldVenueVisitResultDto.fromJson(recordResponse(),
                  command: command())
              .toDomain();
      final observability = RecordingObservabilityService();
      final container = containerFor(repository, observability);
      ...
      expect(container.read(townProvider).town!.venues.single.villagers,
          hasLength(1));
      expect(container.read(townProvider).isRecordingVenueVisit, isFalse);
    });

    test('drops stale command completions and keeps terminal telemetry safe',
        () async {
      final record = Completer<VenueVisitResult>();
      final refreshed = LivingWorldTownDto.fromJson(town(withVillager: false),
              playerId: playerId)
          .toDomain();
      final repository = FakeLivingWorldRepository();
      ...
      final pending =
          container.read(townProvider.notifier).recordVenueVisit(command());
      await container.read(townProvider.notifier).refresh(playerId);
      record.complete(LivingWorldVenueVisitResultDto.fromJson(recordResponse(),
              command: command())
          .toDomain());
      await pending;
    ...
    test('turns unsafe failures into a safe state and telemetry category',
        () async {
      final repository = FakeLivingWorldRepository()
        ..onRead = (_) =>
      ...
      expect(container.read(townProvider).error,
          'Unable to load your Town. Pull to retry.');
      expect(observability.events.join(), isNot(contains('database password')));
