...
import 'package:earth_nova/features/identification/data/repositories/supabase_base_item_content_repository.dart';
import 'package:flutter_test/flutter_test.dart';

void main() {
  final stableId = StableContentId<BaseItemContent>('base-item-1');
  final currentVersionId = ContentVersionId<BaseItemContent>('version-2');
  final retiredVersionId = ContentVersionId<BaseItemContent>('version-1');

  Map<String, Object?> currentRow({
    String ownerId = 'base-item-1',
    String currentId = 'version-2',
    Object? publicationStatus = 'published',
    Object? revision = 2,
    Object? authoredContent = const <String, Object?>{'name': 'Current'},
  }) =>
      <String, Object?>{
        'id': ownerId,
  ...
  group('SupabaseBaseItemContentRepository current lookup', () {
    test('returns the stable row current published exact Version', () async {
      final repository = SupabaseBaseItemContentRepository(
        client: null,
      ...
      expect(content.authoredContent, <String, Object?>{'name': 'Current'});
      expect(
        () => content.authoredContent['name'] = 'Changed',
    ...
    test('rejects malformed current rows', () async {
      final repository = SupabaseBaseItemContentRepository(
        client: null,
        currentQuery: (_) async => currentRow(revision: '2'),
      );
    ...
    test('rejects a current Version with a different stable owner', () async {
      final repository = SupabaseBaseItemContentRepository(
        client: null,
        currentQuery: (_) async => currentRow(ownerId: 'other-base-item'),
      );
  ...
  group('SupabaseBaseItemContentRepository exact lookup', () {
    test('returns a retired exact Version for existing state', () async {
      final repository = SupabaseBaseItemContentRepository(
        ...
        exactQuery: (_) async => exactRow(
          ...
          revision: 1,
          authoredContent: const <String, Object?>{'name': 'Retired'},
        ),
      );
    ...
    test('rejects malformed exact rows and owner mismatches', () async {
      final malformedRepository = SupabaseBaseItemContentRepository(
        client: null,
        exactQuery: (_) async => exactRow(revision: 0),
      );
      final mismatchRepository = SupabaseBaseItemContentRepository(
        client: null,
