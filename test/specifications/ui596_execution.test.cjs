...
test('review evidence remains reproducible without binary PR payloads', () => {
  const reviewDirectory = 'docs/specifications/ui596/review';
  const committedReviewFiles = fs.existsSync(reviewDirectory)
    ? fs.readdirSync(reviewDirectory).filter((name) => /\.(?:png|jpe?g|gif|webp)$/i.test(name))
    : [];
  assert.deepEqual(committedReviewFiles, []);
  assert.doesNotMatch(fs.readFileSync('pubspec.yaml', 'utf8'), /assets\/fonts\/.+\.(?:ttf|otf|woff2?)/);
});
