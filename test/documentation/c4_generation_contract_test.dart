...
void main() {
  test(
    'generated C4 set includes distinct current and target containers',
    () async {
      final check = await Process.run('python3', [
        'scripts/generate_c4_diagrams.py',
        '--check',
      ]);
      ...
      final target = File(
        'docs/c4/02c-container-target.mmd',
      ).readAsStringSync();
      ...
      for (final dynamicView in const [
        '13-dynamic-checkpoint-acceptance.mmd',
        '14-dynamic-whole-save-conflict.mmd',
        '15-dynamic-asynchronous-interaction.mmd',
        '16-dynamic-save-recovery.mmd',
      ]) {
        expect(File('docs/c4/$dynamicView').existsSync(), isTrue);
      }
    },
  );
}
