...
- Optional local programs add extra visible slots with tighter requirements and larger rewards. First/default program style: authored "look for this trait" requests over current animal metadata such as habitat tags, animal type/group, ecological tags, or other existing authored traits — not conservation-status buckets.
- A local program may repeat the same request across multiple slots or mix different habitat/type/trait predicates in one checklist; the slot composition is authored per program.
- Program slot commits are strict: an animal either satisfies the authored predicate or it does not. The UI may suggest nearby/missing matches to guide the player, but suggestions do not grant partial credit.
- Slot occupancy is always one-to-one: one released animal item instance fills exactly one chosen program slot, even if it matches multiple predicates.
- A filled program slot shows the released animal's icon. Tapping/clicking that icon opens the released animal's card, preserving the community-center-style feeling that the contribution is still present.
- Storage model: release commits write an append-only release event as canonical history, then update a current slot occupancy projection for fast UI and one-to-one slot enforcement. Item state still marks the animal as `released`; program/slot display should not rely only on latest-state item columns.
