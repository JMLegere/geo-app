...
Feature: Zoology
  The Field Guide feature needs a concrete zoology section for fauna, threatened species
  meaning, animal behavior, field signs, and migrations.

  Scenario: Zoology defines its app section
    Given Zoology is surfaced through the Field Guide feature
  ...
  Scenario: Player views Zoology guide
    Given the Field Guide has a Zoology section
  ...
  Scenario: Player inspects an animal record
    ...
    Then the Field Guide should show animal facts, observations, habitats, and discovered history
