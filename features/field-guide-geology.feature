...
Feature: Geology
  The Field Guide feature needs a concrete geology section for rock and mineral finds,
  specimen details, and landform context.

  Scenario: Geology defines its app section
    Given Geology is surfaced through the Field Guide feature
  ...
  Scenario: Player views Geology guide
    Given the Field Guide has a Geology section
  ...
  Scenario: Player inspects a rock or mineral record
    ...
    Then the Field Guide should show specimen facts, landform context, and discovered history
