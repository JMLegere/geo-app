...
BEGIN
  ...
  WITH parsed AS (
    ...
    FROM (
      SELECT unnest(ST_ClusterIntersecting(buffered_square)) AS cluster_geom
      FROM base_centroids
    ) clustered
  )
  SELECT
    ...
    base.grid_x,
    base.grid_y,
    base.original_center_lat,
    base.original_center_lng,
    base.organic_center_lat,
    base.organic_center_lng,
    base.centroid_point,
    base.coverage_square,
    cluster_groups.cluster_id
  FROM base_centroids base
  JOIN cluster_groups
    ON ST_Intersects(cluster_groups.cluster_geom, base.buffered_square);
  ...
  WITH assigned AS (
    ...
    JOIN tmp_cell_geometry_cluster_coverage coverage
      ON coverage.cluster_id = voronoi.cluster_id
    LEFT JOIN LATERAL (
      SELECT
        centroids.cell_id,
        centroids.grid_x,
      ...
      FROM tmp_cell_geometry_organic_centroids centroids
      WHERE centroids.cluster_id = voronoi.cluster_id
