-- Geography buffering can leave precision conflicts across otherwise valid
-- cluster coverages. Snap to a sub-decimeter grid and normalize before clustering.
...
BEGIN
  ...
  DROP TABLE IF EXISTS tmp_cell_geometry_cluster_supermembers;
  DROP TABLE IF EXISTS tmp_cell_geometry_cluster_supercoverage;
  ...
  CREATE TEMP TABLE tmp_cell_geometry_cluster_supermembers ON COMMIT DROP AS
  WITH grouped AS (
    SELECT
      row_number() OVER ()::INTEGER AS supercluster_id,
      components.cluster_geom
    FROM (
      SELECT (
        ST_Dump(ST_UnaryUnion(ST_Collect(coverage_geom)))
      ).geom::geometry(Polygon, 4326) AS cluster_geom
      FROM tmp_cell_geometry_cluster_coverage
    ) components
  )
  SELECT
    grouped.supercluster_id,
    coverage.cluster_id
  FROM grouped
  JOIN tmp_cell_geometry_cluster_coverage coverage
    ON ST_Contains(
      grouped.cluster_geom,
      ST_PointOnSurface(coverage.coverage_geom)
    );

  CREATE TEMP TABLE tmp_cell_geometry_cluster_supercoverage ON COMMIT DROP AS
  SELECT
    members.supercluster_id,
    COUNT(*)::INTEGER AS cluster_count,
    SUM(coverage.centroid_count)::INTEGER AS centroid_count,
    ST_Multi(
      ST_UnaryUnion(ST_Collect(coverage.coverage_geom))
    )::geometry(MultiPolygon, 4326) AS coverage_geom
  FROM tmp_cell_geometry_cluster_supermembers members
  JOIN tmp_cell_geometry_cluster_coverage coverage
    ON coverage.cluster_id = members.cluster_id
  GROUP BY members.supercluster_id;

  CREATE INDEX tmp_cell_geometry_cluster_supercoverage_geom_idx
    ON tmp_cell_geometry_cluster_supercoverage USING GIST (coverage_geom);

  SELECT ST_Multi(ST_UnaryUnion(ST_Collect(coverage_geom)))::geometry(MultiPolygon, 4326)
  INTO v_coverage_geom
  FROM tmp_cell_geometry_cluster_supercoverage;
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'supercluster_strategy', 'unary-union-connected-components',
  ...
  SELECT
    superclusters.supercluster_id,
    dumped.geom::geometry(Polygon, 4326) AS geom
  FROM tmp_cell_geometry_cluster_supercoverage superclusters
  CROSS JOIN LATERAL (
    SELECT (ST_Dump(superclusters.coverage_geom)).geom
    WHERE superclusters.centroid_count = 1
    UNION ALL
    SELECT (ST_Dump(ST_VoronoiPolygons(
      (
        SELECT ST_Collect(centroids.centroid_point)
        FROM tmp_cell_geometry_organic_centroids centroids
        JOIN tmp_cell_geometry_cluster_supermembers members
          ON members.cluster_id = centroids.cluster_id
        WHERE members.supercluster_id = superclusters.supercluster_id
      ),
      0.0,
      superclusters.coverage_geom
    ))).geom
    WHERE superclusters.centroid_count > 1
  ) dumped;
  ...
  WITH assigned AS (
    SELECT
      ...
      COALESCE(containing.grid_x, nearest.grid_x) AS grid_x,
      COALESCE(containing.grid_y, nearest.grid_y) AS grid_y,
      COALESCE(containing.original_center_lat, nearest.original_center_lat)
          AS original_center_lat,
      COALESCE(containing.original_center_lng, nearest.original_center_lng)
          AS original_center_lng,
      COALESCE(containing.organic_center_lat, nearest.organic_center_lat)
          AS organic_center_lat,
      COALESCE(containing.organic_center_lng, nearest.organic_center_lng)
          AS organic_center_lng,
      voronoi.supercluster_id,
      voronoi.geom AS voronoi_geom,
      coverage.coverage_geom
    FROM tmp_cell_geometry_cluster_voronoi voronoi
    JOIN tmp_cell_geometry_cluster_supercoverage coverage
      ON coverage.supercluster_id = voronoi.supercluster_id
    LEFT JOIN LATERAL (
      SELECT
        centroids.cell_id,
        centroids.grid_x,
      ...
      JOIN tmp_cell_geometry_cluster_supermembers members
        ON members.cluster_id = centroids.cluster_id
      WHERE members.supercluster_id = voronoi.supercluster_id
        AND ST_Contains(voronoi.geom, centroids.centroid_point)
      ORDER BY centroids.centroid_point <-> ST_PointOnSurface(voronoi.geom)
      LIMIT 1
    ) containing ON true
    LEFT JOIN LATERAL (
      SELECT
    ...
    SELECT
      ...
      jsonb_build_object(
        ...
        'coverage_buffer_meters', v_coverage_buffer_meters,
        'cluster_eps_degrees', v_cluster_eps_degrees,
        'supercluster_strategy', 'merge-overlapping-buffered-cluster-coverages',
        'grid_scale', v_grid_scale
      ) AS raw_properties,
      CASE
...
DO $stage_production_voronoi$
DECLARE
  v_active_source TEXT;
...
BEGIN
  ...
  IF v_active_source = 'db-lattice-voronoi-production-v1' THEN
    PERFORM stage_cell_geometry_from_organic_centroids(
      'organic-voronoi-production-v2',
      'earthnova-organic-centroids-prod-v2',
      'db-organic-voronoi-from-deterministic-centroids',
      0.34,
      250.0
    );
    ...
    FROM (
      SELECT cell_id FROM cell_geometry_current
      EXCEPT
      SELECT cell_id
      FROM cell_geometry_staging
      WHERE source_version = 'organic-voronoi-production-v2'
    ) missing;
    ...
    IF v_missing_count <> 0 OR v_extra_count <> 0 THEN
      RAISE EXCEPTION
        'Organic Voronoi candidate changed the production cell ID set: missing %, extra %',
        v_missing_count,
        v_extra_count;
    END IF;

    v_validation_run_id := validate_cell_geometry_source_version(
      'organic-voronoi-production-v2',
      1.0,
      1.0
    );
    ...
    IF NOT EXISTS (
      ...
      WHERE id = v_validation_run_id
        AND source_version = 'organic-voronoi-production-v2'
        AND status = 'passed'
    ) THEN
      RAISE EXCEPTION
        'Organic Voronoi candidate validation did not pass: %',
        v_validation_run_id;
    END IF;
  END IF;
END
$stage_production_voronoi$;
