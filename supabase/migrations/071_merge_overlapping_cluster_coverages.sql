...
BEGIN
  ...
  WITH grouped AS (
    SELECT
      ...
      cluster_geom::geometry(GeometryCollection, 4326) AS cluster_geom
    FROM (
      SELECT unnest(ST_ClusterIntersecting(coverage_geom)) AS cluster_geom
  ...
  JOIN tmp_cell_geometry_cluster_coverage coverage
    ON ST_Intersects(grouped.cluster_geom, coverage.coverage_geom);
