...
AS $$
  WITH focus_point AS (
    ...
    SELECT current.cell_id, current.centroid, current.geom
    FROM cell_geometry_current current
  ), focus_cells AS (
    SELECT active.cell_id
    FROM active_cells active
    CROSS JOIN focus_point focus
    CROSS JOIN active_boundary boundary
    WHERE ST_DWithin(active.centroid::geography, focus.geom, p_focus_radius_meters)
      AND ST_DWithin(active.centroid::geography, boundary.geom, p_boundary_band_meters)
  ), site_cells AS (
    SELECT active.cell_id
    FROM active_cells active
    CROSS JOIN focus_point focus
    WHERE ST_DWithin(active.centroid::geography, focus.geom, p_site_radius_meters)
  ), jittered AS (
    SELECT
      cp.cell_id,
      ...
      split_part(cp.cell_id, '_', 3)::INTEGER / 500.0 +
        ((cell_geometry_hash_unit(cp.cell_id || ':' || p_centroid_dataset_version || ':lat') * 2.0 - 1.0) * (p_jitter_ratio / 500.0))
        AS organic_center_lat,
      split_part(cp.cell_id, '_', 2)::INTEGER / 500.0 +
        ((cell_geometry_hash_unit(cp.cell_id || ':' || p_centroid_dataset_version || ':lng') * 2.0 - 1.0) * (p_jitter_ratio / 500.0))
        AS organic_center_lng
    FROM cell_properties cp
    JOIN site_cells site
      ON site.cell_id = cp.cell_id
