...
function validateManifest(manifest, plan, expectedIds) {
  const errors = [];
  const decisions = [...manifest.questions, ...manifest.baseline];
  const seen = new Set();
...
async function main() {
  ...
  for (const scenario of manifest.scenarios) {
    ...
    for (const decision of [...manifest.questions, ...manifest.baseline]) {
  ...
  console.log("Parsed " + sources.plan.length + " target scenarios; traced 111 questions and 27 baseline records.");
