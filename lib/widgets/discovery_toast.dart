...
import 'package:earth_nova/models/discovery_event.dart';
...
class _DiscoveryNotificationOverlayState
  ...
  void _startDismissTimer() {
    ...
    _dismissTimer = Timer(Durations.discoveryToast, () {
      if (mounted) {
        ref.read(discoveryProvider.notifier).dismissToast();
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ref.listen<DiscoveryState>(discoveryProvider, (previous, next) {
      final wasActive = previous?.activeToast != null;
      final isActive = next.activeToast != null;
      ...
      if (isActive && !wasActive) {
        ...
        final prevTop = previous?.activeToast;
        final nextTop = next.activeToast;
        if (prevTop != nextTop) {
          _startDismissTimer();
        }
      }
    ...
    // Only show DiscoveryEvent-typed toasts in this overlay.
    final discoveryQueue = queue.whereType<DiscoveryEvent>().toList();

    if (discoveryQueue.isEmpty) return const SizedBox.shrink();

    final visibleCount = math.min(discoveryQueue.length, _kMaxVisibleCards);
