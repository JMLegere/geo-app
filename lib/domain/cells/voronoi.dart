...
import 'package:earth_nova/domain/cells/delaunay.dart';
...
class LazyVoronoiCellService implements CellService {
  ...
  List<Geographic> getCellBoundary(String cellId) {
    ...
    return data.boundary.map((p) => Geographic(lat: p.$1, lon: p.$2)).toList();
  }
  ...
  _CellVoronoiData _computeLocalVoronoi(int row, int col) {
    ...
    final boundary = voronoiVertices.map((v) => (v.$1, v.$2)).toList();

    final neighborIds =
        neighborGridKeys.map((key) => _cellId(key.$1, key.$2)).toList()..sort();
