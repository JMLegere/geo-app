...
The first pass establishes one design system for both reusable components and app-specific UI surfaces. New reusable UI should enter the taxonomy; app-specific screens/widgets/painters must still be inventoried as design surfaces.
...
`designSurfaceInventory`. That inventory is the single enforcement path for
app-specific UI: no side allowlists, no exception table, and no silent
local styling.
