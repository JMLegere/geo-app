...
class GameCoordinator {
  ...
  /// Optional observability buffer for analytics/telemetry.
  /// When set, game events are emitted alongside existing callbacks.
  final ObservabilityBuffer? obs;
    ...
    if (rawGps != null) {
      ...
      if (!isNearby) {
        ...
        if (!_explorationDisabled) {
          ...
          obs?.event('exploration_disabled_changed', {
            'disabled': true,
          });
        }
  ...
  void _resolveCellProperties(double lat, double lon) {
    ...
    for (final cellId in cellIds) {
      ...
      obs?.event('cell_properties_resolved', {
