...
class CellProperties {
  ...
  factory CellProperties.fromDrift(LocalCellProperties row) {
    return CellProperties(
      cellId: row.cellId,
      habitats: (jsonDecode(row.habitats) as List)
  ...
  LocalCellProperties toDriftRow() {
    return LocalCellProperties(
      cellId: cellId,
      habitats: jsonEncode(habitats.map((h) => h.name).toList()),
      climate: climate.name,
      continent: continent.name,
      locationId: locationId,
      createdAt: createdAt,
  ...
  LocalCellPropertiesTableCompanion toDriftCompanion() {
    return LocalCellPropertiesTableCompanion(
      cellId: Value(cellId),
      habitats: Value(jsonEncode(habitats.map((h) => h.name).toList())),
